...
object FirrtlEquivalenceTestUtils {
  ...
  def firrtlEquivalenceTestPass(
    ...
    val customResult = customTransforms.foldLeft(
      CircuitState(
        circuit,
        ChirrtlForm,
    ...
    val referenceResult = referenceTransforms.foldLeft(
      CircuitState(
        circuit,
        ChirrtlForm,
...
import ExprGen._
class InlineBooleanExprsCircuitGenerator
...
class FirrtlEquivalenceTests {
  ...
  private def runTest(c: Circuit, referenceCompiler: TransformManager, customCompiler: TransformManager) = {
    ...
    if (!passed) {
      Assert.assertTrue(s"not equivalent to reference compiler on input ${testDir}:\n${c.serialize}\n", false)
    }
  }
