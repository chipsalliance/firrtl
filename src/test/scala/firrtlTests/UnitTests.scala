...
class UnitTests extends FirrtlFlatSpec {
  ...
  "Simple compound expressions" should "be split" in {
    val passes = Seq(ToWorkingIR, ResolveKinds, InferTypes, ResolveFlows, new InferWidths, SplitExpressions)
    val input =
      """circuit Top :
        |  module Top :
        |    input a : UInt<32>
    ...
    val check = Seq("node _GEN_0 = add(a, b)", "c <= geq(_GEN_0, d)")
    executeTest(input, check, passes)
  }

  "Smaller widths" should "be explicitly padded" in {
    val passes = Seq(ToWorkingIR, ResolveKinds, InferTypes, ResolveFlows, new InferWidths, PadWidths)
    val input =
      """circuit Top :
        |  module Top :
        |    input a : UInt<32>
  ...
  "Verilog SInts" should "work" in {
    ...
    val input =
      """circuit Unit :
        ...
        |    negSInt <= SInt(-13)
        |""".stripMargin
    val expected = Seq("""assign posSInt = 5'shd;""", """assign negSInt = -5'shd;""")
