...
class InequalitySpec extends AnyFlatSpec with Matchers {
  ...
  "IsMax" should "reduce properly" in {
    ...
    IsMax(Seq(Closed(2), IsMin(Closed(1), VarCon("a")), IsMin(Closed(3), VarCon("b")))) should be(
      IsMax(Seq(Closed(2), IsMin(Closed(3), VarCon("b"))))
    )
  ...
  "IsMin" should "reduce properly" in {
    ...
    IsMin(Seq(Closed(2), IsMax(Closed(1), VarCon("a")), IsMax(Closed(3), VarCon("b")))) should be(
      IsMin(Seq(Closed(2), IsMax(Closed(1), VarCon("a"))))
    )
  ...
  "IsMul" should "reduce properly" in {
    ...
    IsMul(Closed(2), IsMax(Closed(3), VarCon("a"))) should be(IsMax(Closed(6), IsMul(Closed(2), VarCon("a"))))

    // Pull out max, if negative is min
    IsMul(Closed(-2), IsMax(Closed(3), VarCon("a"))) should be(IsMin(Closed(-6), IsMul(Closed(-2), VarCon("a"))))

    // Pull out min, if positive stays min
    IsMul(Closed(2), IsMin(Closed(3), VarCon("a"))) should be(IsMin(Closed(6), IsMul(Closed(2), VarCon("a"))))

    // Pull out min, if negative is max
    IsMul(Closed(-2), IsMin(Closed(3), VarCon("a"))) should be(IsMax(Closed(-6), IsMul(Closed(-2), VarCon("a"))))
