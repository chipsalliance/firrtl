...
class UniquifySpec extends FirrtlFlatSpec {
  ...
  private def transforms = Seq(ToWorkingIR, CheckHighForm, ResolveKinds, InferTypes, Uniquify)
  ...
  it should "rename DefRegister expressions: clock, reset, and init" in {
    val input =
      ...
      """.stripMargin
    val expected = Seq("reg foo : UInt<4>, clock_[1] with :", "reset => (reset_.a, init_[3].b_[1].d)").map(normalized)
  ...
  it should "rename ports before statements" in {
    val input =
      ...
      """.stripMargin
    val expected = Seq("input data : { a : UInt<4>, b : UInt<4>}[2]", "node data_0_a_ = data[0].a").map(normalized)
