...
class LowerTypesSpec extends FirrtlFlatSpec {
  ...
  it should "lower mixed-direction ports" in {
    val input =
      """circuit Test :
        ...
        |    foo is invalid
      """.stripMargin
    val expected = Seq("output foo_0_a : UInt<1>", "input foo_0_b : UInt<1>").map(normalized)
  ...
  it should "lower DefRegister expressions: clock, reset, and init" in {
    val input =
      ...
      """.stripMargin
    val expected = Seq("reg foo : UInt<4>, clock_1 with :", "reset => (reset_a, init_3_b_1_d)").map(normalized)
