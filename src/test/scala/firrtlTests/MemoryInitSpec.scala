...
class MemInitSpec extends FirrtlFlatSpec {
  ...
  "MemoryScalarInitAnnotation on Memory with Bundle type" should "fail" in {
    ...
    assert(
      caught.getMessage
        .endsWith("[module MemTest] Cannot initialize memory of non ground type { real : SInt<10>, imag : SInt<10>}")
    )
  }
...
abstract class MemInitExecutionSpec(values: Seq[Int], init: ReferenceTarget => Annotation)
  ...
  override def commands: Seq[SimpleTestCommand] =
    (Seq(-1) ++ values).zipWithIndex.map {
      case (value, addr) =>
...
class MemScalarInit0ExecutionSpec
    extends MemInitExecutionSpec(Seq.tabulate(31)(_ => 0), r => MemoryScalarInitAnnotation(r, 0)) {}

class MemScalarInit17ExecutionSpec
    extends MemInitExecutionSpec(Seq.tabulate(31)(_ => 17), r => MemoryScalarInitAnnotation(r, 17)) {}
