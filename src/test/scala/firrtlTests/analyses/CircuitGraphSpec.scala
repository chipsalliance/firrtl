...
class CircuitGraphSpec extends FirrtlFlatSpec {
  "CircuitGraph" should "build the graph for rocket-chip" in {
    CircuitGraph(
      new TransformManager(Seq(), Seq(Dependency[ExpandWhensAndCheck]))
        .execute(
          CircuitState(
            parse(FileUtils.getTextResource("/regress/RocketCore.fir")),
            UnknownForm
          )
        )
        .circuit
  ...
  //      input ++= mkLeaf(n)
  //      val circuit = new TransformManager(Seq(), Seq()).execute(CircuitState(parse(input.toString()), ChirrtlForm)).circuit
