...
class DedupModuleTests extends HighTransformSpec {
  ...
  "The module A and A_" should "be deduped with same annotation targets" in {
    ...
    val cs = execute(
      input,
      check,
      Seq(dontTouch(ReferenceTarget("Top", "A", Nil, "b", Nil)), dontTouch(ReferenceTarget("Top", "A_", Nil, "b", Nil)))
    )
  ...
  "The module A and A_" should "be deduped with same annotations with same multi-targets" in {
    ...
    cs.annotations.toSeq should contain(MultiTargetDummyAnnotation(Seq(Top_a1, Top_a1_b), 0))
    cs.annotations.toSeq should contain(MultiTargetDummyAnnotation(Seq(Top_a2, Top_a2_b), 1))
  }
  "The module A and A_" should "be deduped with same annotations with same multi-targets, that share roots" in {
    ...
    cs.annotations.toSeq should contain(
      MultiTargetDummyAnnotation(
        Seq(Top.module("Top").instOf("a1", "A"), Top.module("Top").instOf("a1", "A").instOf("b", "B")),
        0
      )
    )
    cs.annotations.toSeq should contain(
      MultiTargetDummyAnnotation(
        Seq(Top.module("Top").instOf("a2", "A"), Top.module("Top").instOf("a2", "A").instOf("b", "B")),
        1
      )
    )
  ...
  "The deduping module A and A_" should "rename instances and signals that have different names" in {
    ...
    cs.annotations.toSeq should contain(MultiTargetDummyAnnotation(Seq(inst1, ref1), 0))
    cs.annotations.toSeq should contain(
      MultiTargetDummyAnnotation(
        Seq(
