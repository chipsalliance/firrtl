...
class GroupComponentsSpec extends MiddleTransformSpec {
  ...
  "The register r" should "be grouped" in {
    val input =
      s"""circuit $top :
         ...
         |    out <= r
      """.stripMargin
    val groups = Seq(GroupAnnotation(Seq(topComp("r")), "MyReg", "rInst", Some("_OUT"), Some("_IN")))
    val check =
      s"""circuit Top :
         |  module $top :
         |    input clk: Clock
  ...
  "Grouping" should "work even when there are unused nodes" in {
    val input =
      s"""circuit $top :
         ...
         |    out <= w
      """.stripMargin
    val groups = Seq(GroupAnnotation(Seq(topComp("w")), "Child", "inst", Some("_OUT"), Some("_IN")))
    val check =
      s"""circuit Top :
         |  module $top :
         |    input in: UInt<16>
  ...
  "Instances with uninitialized ports" should "work properly" in {
    val input =
      ...
      """.stripMargin
    val groups = Seq(GroupAnnotation(Seq(topComp("other")), "Wrapper", "wrapper"))
    val check =
      s"""circuit $top :
         |  module $top :
         |    input in: UInt<16>
  ...
  "Grouping" should "properly set kinds" in {
    val input =
      """circuit Top :
        ...
        |    out <= r
      """.stripMargin
    val groups = Seq(GroupAnnotation(Seq(topComp("r")), "MyModule", "inst", Some("_OUT"), Some("_IN")))
    val result =
      (new VerilogCompiler).compileAndEmit(CircuitState(parse(input), ChirrtlForm, groups), Seq(new GroupComponents))
