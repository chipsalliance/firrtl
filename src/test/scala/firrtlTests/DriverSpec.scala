...
class DriverSpec extends AnyFreeSpec with Matchers with BackendCompilationUtilities {
  ...
  "FirrtlOptions holds option information for the firrtl compiler" - {
    ...
    "input and output file names can be overridden, overrides do not use targetDir" in {
      ...
      optionsManager.parse(Array("--top-name", "cat", "-i", "./bob.fir", "-o", "carol.v")) should be(true)
    ...
    "various annotations can be created from command line, currently:" - {
      "inline annotation" in {
        ...
        optionsManager.parse(Array("--inline", "module,module.submodule,module.submodule.instance")) should be(true)
      ...
      "infer-rw annotation" in {
        ...
        optionsManager.parse(Array("--infer-rw")) should be(true)
      ...
      "repl-seq-mem annotation" in {
        ...
        optionsManager.parse(Array("--repl-seq-mem", "-c:circuit1:-i:infile1:-o:outfile1")) should be(true)
  ...
  "Annotations can be read using annotationFileNameOverride" in {
    val optionsManager = new ExecutionOptionsManager("test") with HasFirrtlOptions {
      ...
      firrtlOptions = firrtlOptions.copy(annotationFileNameOverride = "SampleAnnotations")
    }
    val annotationsTestFile = new File(
      optionsManager.commonOptions.targetDirName,
      optionsManager.firrtlOptions.annotationFileNameOverride + ".anno"
    )
  ...
  "Supported LegacyAnnotations will be converted automagically" in {
    ...
    val optionsManager = new ExecutionOptionsManager("test") with HasFirrtlOptions {
      ...
      firrtlOptions = firrtlOptions.copy(annotationFileNames = List(annotationsTestFile.toString))
    }
  ...
  "UNsupported LegacyAnnotations should throw errors" in {
    ...
    for ((anno, i) <- yamlAnnos.zipWithIndex) {
      ...
      val optionsManager = new ExecutionOptionsManager("test") with HasFirrtlOptions {
        commonOptions = commonOptions.copy(topName = "test", targetDirName = testDir.toString)
        firrtlOptions = firrtlOptions.copy(annotationFileNames = List(annoFile.toString))
      }
  ...
  "Annotations can be read from multiple files" in {
    ...
    val optionsManager = new ExecutionOptionsManager("test") with HasFirrtlOptions {
      commonOptions = commonOptions.copy(topName = "a.fir")
      firrtlOptions = firrtlOptions.copy(annotationFileNames =
        List.fill(2)(filename) // just read the safe file twice
      )
    }
  ...
  "Annotations can be created from the command line and read from a file at the same time" in {
    ...
    optionsManager.parse(Array("--infer-rw", "-faf", annoFile.toString)) should be(true)
