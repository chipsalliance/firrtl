...
object Serializer {
  ...
  private def flattenInfo(infos: Seq[Info]): Seq[FileInfo] =
    infos.flatMap {
      case NoInfo => Seq()
  ...
  private def s(node: Info)(implicit b: StringBuilder, indent: Int): Unit =
    node match {
  ...
  private def s(node: Expression)(implicit b: StringBuilder, indent: Int): Unit =
    node match {
  ...
  private def s(node: Statement)(implicit b: StringBuilder, indent: Int): Unit =
    node match {
  ...
  private def s(node: Width)(implicit b: StringBuilder, indent: Int): Unit =
    node match {
  ...
  private def sPoint(node: Width)(implicit b: StringBuilder, indent: Int): Unit =
    node match {
      case IntWidth(width) => b ++= "<<"; b ++= width.toString(); b ++= ">>"
      case UnknownWidth    => // empty string
      case CalcWidth(arg)  => b ++= "calcw("; s(arg); b += ')'
  ...
  private def s(node: Orientation)(implicit b: StringBuilder, indent: Int): Unit =
    node match {
      case Default => // empty string
      case Flip    => b ++= "flip "
    }

  private def s(node: Field)(implicit b: StringBuilder, indent: Int): Unit =
    node match {
      case Field(name, flip, tpe) => s(flip); b ++= name; b ++= " : "; s(tpe)
    }

  private def s(node: Type)(implicit b: StringBuilder, indent: Int): Unit =
    node match {
      // Types
  ...
  private def s(node: Direction)(implicit b: StringBuilder, indent: Int): Unit =
    node match {
      case Input  => b ++= "input"
      case Output => b ++= "output"
    }

  private def s(node: Port)(implicit b: StringBuilder, indent: Int): Unit =
    node match {
      case Port(info, name, direction, tpe) =>
        s(direction); b += ' '; b ++= name; b ++= " : "; s(tpe); s(info)
    }

  private def s(node: Param)(implicit b: StringBuilder, indent: Int): Unit =
    node match {
  ...
  private def s(node: DefModule)(implicit b: StringBuilder, indent: Int): Unit =
    node match {
  ...
  private def s(node: Circuit)(implicit b: StringBuilder, indent: Int): Unit =
    node match {
  ...
  private def s(const: Constraint)(implicit b: StringBuilder): Unit =
    const match {
      // Bounds
      case UnknownBound   => b += '?'
  ...
  private def s(nodes: Iterable[Expression], sep: String, noFinalSep: Boolean = true)(implicit
    b:                 StringBuilder,
    indent:            Int
  ): Unit = {
