...
case class MultiInfo(infos: Seq[Info]) extends Info {
  private def collectStrings(info: Info): Seq[String] =
    info match {
...
object MultiInfo {
  ...
  private[firrtl] def demux(info: Info): (Info, Info, Info) =
    info match {
...
object StringLit {
  ...
  private def toASCII(char: Char): List[Char] =
    char match {
...
case class SubIndex(expr: Expression, value: Int, tpe: Type, flow: Flow = UnknownFlow) extends Expression {
  def mapExpr(f:      Expression => Expression): Expression = this.copy(expr = f(expr))
  def mapType(f:      Type => Type):             Expression = this.copy(tpe = f(tpe))
  def mapWidth(f:     Width => Width):           Expression = this
  def foreachExpr(f:  Expression => Unit):       Unit = f(expr)
...
case class SubAccess(expr: Expression, index: Expression, tpe: Type, flow: Flow = UnknownFlow) extends Expression {
...
case class DefRegister(info: Info, name: String, tpe: Type, clock: Expression, reset: Expression, init: Expression)
    extends Statement
    with IsDeclaration {
  def mapStmt(f: Statement => Statement): Statement = this
  def mapExpr(f: Expression => Expression): Statement =
...
case class Verification(op: Formal.Value, info: Info, clk: Expression, pred: Expression, en: Expression, msg: StringLit)
    extends Statement
    with HasInfo {
  def mapStmt(f: Statement => Statement): Statement = this
  def mapExpr(f: Expression => Expression): Statement =
...
object KnownBound {
  ...
  def unapply(b: Bound): Option[BigDecimal] =
    b match {
      case k: IsKnown => Some(k.value)
      case _ => None
    }
...
case class IntervalType(lower: Bound, upper: Bound, point: Width) extends GroundType {
  ...
  private def precision: Option[BigDecimal] =
    point match {
      case IntWidth(width) =>
        val bp = width.toInt
  ...
  def min: Option[BigDecimal] =
    (lower, precision) match {
  ...
  def minAdjusted: Option[BigInt] =
    min.map(_ * BigDecimal(BigInt(1) << bp) match {
