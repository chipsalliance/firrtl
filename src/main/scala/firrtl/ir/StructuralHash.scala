...
object StructuralHash {
  ...
  private def hash(node: FirrtlNode, h: Hasher, rename: String => String): Unit =
    node match {
  ...
  private def hashPortTypeName(tpe: Type, h: String => Unit): Unit =
    tpe match {
  ...
  private def verificationOp(op: Formal.Value): Byte =
    op match {
...
class StructuralHash private (h: Hasher, renameModule: String => String) {
  ...
  @inline private def n(name: String): Unit =
    hash(
  ...
  private def hash(node: Expression): Unit =
    node match {
  ...
  private def hash(node: Statement): Unit =
    node match {
  ...
  private def hash(ruw: ReadUnderWrite.Value): Unit =
    ruw match {
  ...
  private def hash(node: Width): Unit =
    node match {
  ...
  private def hash(node: Orientation): Unit =
    node match {
      case Default => id(46)
      case Flip    => id(47)
    }
  ...
  private def hash(node: Type): Unit =
    node match {
      // Types
  ...
  private def hash(node: Direction): Unit =
    node match {
      case Input  => id(66)
      case Output => id(67)
    }
  ...
  private def hash(node: Param): Unit =
    node match {
  ...
  private def hash(node: DefModule): Unit =
    node match {
  ...
  private def hash(d: firrtl.MPortDir): Unit =
    d match {
  ...
  private def hash(c: firrtl.constraint.Constraint): Unit =
    c match {
