...
object ConvertFixedToSInt extends Pass {
  ...
  def alignArg(e: Expression, point: BigInt): Expression =
    e.tpe match {
  ...
  def run(c: Circuit): Circuit = {
    ...
    def onModule(m: DefModule): DefModule = {
      ...
      def updateExpType(e: Expression): Expression =
        e match {
      ...
      def updateStmtType(s: Statement): Statement =
        s match {
