...
object RemoveCHIRRTL extends Transform {
  ...
  def create_all_exps(ex: Expression): Seq[Expression] = ex.tpe match {
    ...
    case t: BundleType =>
      t.fields.foldLeft(Seq[Expression]())((exps, f) => exps ++ create_all_exps(SubField(ex, f.name, f.tpe))) ++ Seq(ex)
    case t: VectorType =>
      (0 until t.size).foldLeft(Seq[Expression]())((exps, i) => exps ++ create_all_exps(SubIndex(ex, i, t.tpe))) ++ Seq(
        ex
      )
  ...
  def collect_refs(
    ...
    case sx: CDefMemory =>
      ...
      def set_enable(vec: Seq[MPort], en: String) =
        vec.map(r => Connect(sx.info, SubField(SubField(Reference(sx.name, ut), r.name, ut), en, BoolType), zero))
