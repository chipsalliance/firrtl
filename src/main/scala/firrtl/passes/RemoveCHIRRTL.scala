...
object RemoveCHIRRTL extends Transform with DependencyAPIMigration {
  ...
  override def prerequisites =
    firrtl.stage.Forms.ChirrtlForm ++
  ...
  def create_all_exps(ex: Expression): Seq[Expression] =
    ex.tpe match {
      case _: GroundType => Seq(ex)
      case t: BundleType =>
        (t.fields.foldLeft(Seq[Expression]()))((exps, f) =>
          exps ++ create_all_exps(SubField(ex, f.name, f.tpe))
        ) ++ Seq(ex)
      case t: VectorType =>
  ...
  def collect_refs(
    ...
    s match {
      case sx: CDefMemory =>
        ...
        val tdata = sx.tpe
        def set_poison(vec: scala.collection.Seq[MPort]) =
          vec.toSeq.flatMap(r =>
            Seq(
        ...
        def set_write(vec: scala.collection.Seq[MPort], data: String, mask: String) =
          vec.toSeq.flatMap { r =>
