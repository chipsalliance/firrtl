...
object ZeroWidth extends Transform with DependencyAPIMigration {
  ...
  private def onEmptyMemStmt(s: Statement): Statement =
    s match {
  ...
  private def findRemovable(expr: => Expression, tpe: Type): Seq[Expression] =
    tpe match {
  ...
  private[passes] def removeZero(t: Type): Option[Type] =
    t match {
  ...
  private def onStmt(renames: RenameMap)(s: Statement): Statement =
    s match {
