...
class InferWidths extends Transform with ResolvedAnnotationPaths with DependencyAPIMigration {
  ...
  private def addTypeConstraints(r1: ReferenceTarget, r2: ReferenceTarget)(t1: Type, t2: Type)(implicit
  ...
  private def addStmtConstraints(
    mt: ModuleTarget
  )(s:  Statement)(implicit constraintSolver: ConstraintSolver): Statement =
    s.map(addExpConstraints) match {
      case c: Connect =>
  ...
  private def fixWidth(w: Width)(implicit constraintSolver: ConstraintSolver): Width =
    constraintSolver.get(w) match {
      case Some(Closed(x)) if trim(x).isWhole => IntWidth(x.toBigInt)
      case None                               => w
      case _                                  => sys.error("Shouldn't be here")
    }
  private def fixType(t: Type)(implicit constraintSolver: ConstraintSolver): Type =
    t.map(fixType).map(fixWidth) match {
