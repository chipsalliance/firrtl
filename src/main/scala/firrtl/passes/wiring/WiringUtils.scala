...
case class Modifications(addPortOrWire: Option[(String, DecKind)] = None, cons: Seq[(String, String)] = Seq.empty) {
...
case class Lineage(
  ...
  def shortSerialize(tab: String): String =
    s"""
       |$tab name: $name,
...
object WiringUtils {
  ...
  def getChildrenMap(c: Circuit): ChildrenMap = {
    ...
    def getChildren(mname: String)(s: Statement): Unit =
      s match {
        case s: WDefInstance =>
  ...
  /** Return a map of sink instances to source instances that minimizes
    ...
    */
  @deprecated(
    "This method can lead to non-determinism in your compiler pass. Use sinksToSourcesSeq instead!",
    "Firrtl 1.4"
  )
  ...
  def getType(c: Circuit, module: String, comp: String): Type = {
    def getRoot(e: Expression): String =
      e match {
    ...
    def getType(s: Statement): Statement =
      s match {
    ...
    tpe match {
      ...
      case Some(t) =>
        def setType(e: Expression): Expression =
          e.map(setType) match {
