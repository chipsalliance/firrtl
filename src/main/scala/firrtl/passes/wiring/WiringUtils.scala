...
case class Lineage(
  ...
  def shortSerialize(tab: String): String =
    s"""
       |$tab name: $name,
       |$tab children: ${children.map(
         c => tab + "   " + c._2.shortSerialize(tab + "    ")
       )}
       |""".stripMargin
...
object WiringUtils {
  ...
  def sinksToSources(sinks: Seq[Named], source: String, i: InstanceGraph): Map[Seq[WDefInstance], Seq[WDefInstance]] = {
    ...
    i.fullHierarchy.keys.filter { case WDefInstance(_, _, m, _) => m == source }
      .foreach(i.fullHierarchy(_).foreach { l =>
        queue.enqueue(l)
        owners(l) = Vector(l)
      })

    val sinkInsts = i.fullHierarchy.keys.filter {
