...
object Uniquify extends Transform with DependencyAPIMigration {
  ...
  /** Enumerates all possible names for a given type. For example:
    ...
    * }}}
    */
  private[firrtl] def enumerateNames(tpe: Type): Seq[Seq[String]] =
    tpe match {
      case t: BundleType =>
        t.fields.flatMap { f =>
  ...
  def stmtToType(s: Statement)(implicit sinfo: Info, mname: String): BundleType = {
    // Recursive helper
    def recStmtToType(s: Statement): Seq[Field] =
      s match {
  ...
  private def uniquifyNames(t: BundleType, namespace: collection.mutable.HashSet[String])(implicit
    sinfo:                     Info,
    mname:                     String
  ): BundleType = {
    def recUniquifyNames(t: Type, namespace: collection.mutable.HashSet[String]): (Type, Seq[String]) =
      t match {
        case tx: BundleType =>
  ...
  private def createNameMapping(from: Type, to: Type)(implicit sinfo: Info, mname: String): Map[String, NameMapNode] = {
    (from, to) match {
  ...
  private def uniquifyNamesExp(exp: Expression, map: Map[String, NameMapNode])(implicit
    sinfo:                          Info,
    mname:                          String
  ): Expression = {
    // Recursive Helper
    def rec(exp: Expression, m: Map[String, NameMapNode]): (Expression, Map[String, NameMapNode]) =
      exp match {
        case e: WRef =>
  ...
  private def uniquifyNamesType(tpe: Type, map: Map[String, NameMapNode])(implicit sinfo: Info, mname: String): Type =
    tpe match {
      case t: BundleType =>
  ...
  def execute(state: CircuitState): CircuitState = {
    ...
    def uniquifyModule(renames: RenameMap)(m: DefModule): DefModule = {
      ...
      def uniquifyExp(e: Expression): Expression =
        e match {
