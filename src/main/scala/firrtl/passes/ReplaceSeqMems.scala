  ...
  def flattenMemVec(dataBlockWidth:BigInt, e: Expression): Seq[Expression] = {
    Utils.tpe(e) match {
      ...
      case t: UIntType => {
        ...
        val ref = e.asInstanceOf[WSubField].exp
        val name = e.asInstanceOf[WSubField].name
        if (name == "mask" || name == "wmask") Seq(WSubField(ref,"NOTUSED",t,gender(e)))
        else Seq(e)
      }
      case t => Seq(e) 
    }
  ...
  def map_connect (intPorts:Expression, extRef: WRef, nameMap: Map[String,String], blockWidth: Int) : Seq[Statement] = {
    ...
    tpe(intPorts) match {
      ...
      case t: VectorType => {
        ...
        //}
        if (name == "data" || name == "rdata" || name == "wdata" || name == "mask" || name =="wmask"){
      ...
      case t => {
        ...
        val ignoreMask = (lowerExtName(intPorts).split("_").last == "mask") || lowerExtName(intPorts).split("_").last == "wmask"
        if (!ignoreMask){
