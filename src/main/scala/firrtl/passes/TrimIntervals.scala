...
class TrimIntervals extends Pass {
  ...
  private def replaceTypeInterval(t: Type): Type =
    t match {
  ...
  private def alignStmtBP(s: Statement): Statement =
    s.map(alignExpBP) match {
  ...
  private def alignExpBP(e: Expression): Expression =
    e.map(alignExpBP) match {
  ...
  private def fixBP(p: Width)(e: Expression): Expression =
    (p, e.tpe) match {
