...
class InferBinaryPoints extends Pass {
  ...
  override def prerequisites =
    Seq(Dependency(ResolveKinds), Dependency(InferTypes), Dependency(Uniquify), Dependency(ResolveFlows))
  ...
  private def addStmtConstraints(mt: ModuleTarget)(s: Statement): Statement =
    s.map(addDecConstraints) match {
      case c: Connect =>
