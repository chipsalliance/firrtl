...
class InlineInstances extends Transform with RegisteredTransform {
  ...
  def check(c: Circuit, moduleNames: Set[ModuleName], instanceNames: Set[ComponentName]): Unit = {
    ...
    moduleNames.foreach { mn =>
      checkExternal(mn.name)
    }
    if (errors.nonEmpty) throw new PassExceptions(errors)
    instanceNames.foreach { cn =>
      checkInstance(cn)
    }
    if (errors.nonEmpty) throw new PassExceptions(errors)
  }
  ...
  def run(
    ...
    val flatInstances
      : Set[(OfModule, Instance)] = instsToInline.map(i => OfModule(i.module.name) -> Instance(i.name)) ++ getInstancesOf(
      c,
      flatModules
    )
    ...
    def appendRefPrefix(currentModule: IsModule, renames: mutable.HashMap[String, String])(s: Statement): Statement = {
    ...
    def fixupRefs(instMap: collection.Map[Instance, OfModule], currentModule: IsModule)(e: Expression): Expression = {
      e match {
    ...
    def onStmt(currentModule: ModuleTarget)(s: Statement): Statement = {
      ...
      s match {
        case wDef @ WDefInstance(_, instName, modName, _)
          ...
          renamedBody
        case sx =>
          sx.map(fixupRefs(instMap, currentModule))
