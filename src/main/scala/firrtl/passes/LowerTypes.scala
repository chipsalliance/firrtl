...
object LowerTypes extends Transform with DependencyAPIMigration {
  ...
  private def onStatement(
    s:                Statement
  )(implicit symbols: LoweringTable, memInit: Seq[MemoryInitAnnotation]): Statement =
    s match {
      // declarations
      case d: DefWire =>
        Block(
          symbols.lower(d.name, d.tpe, firrtl.WireKind).map { case (name, tpe, _) => d.copy(name = name, tpe = tpe) }
        )
  ...
  private def onExpression(e: Expression)(implicit symbols: LoweringTable): Expression =
    e match {
...
private class LoweringTable(
  ...
  private def serialize(expr: RefLikeExpression): String =
    expr match {
...
private object DestructTypes {
  ...
  private def memBundle(mem: DefMemory): Field =
    mem.dataType match {
  ...
  private def getFields(tpe: Type): Seq[Field] =
    tpe match {
