...
object LowerTypes extends Transform with DependencyAPIMigration {
  ...
  def renameExps(renames: RenameMap, e: Expression, root: String): Seq[String] =
    e.tpe match {
      case (_: GroundType) =>
  ...
  private def renameMemExps(renames: RenameMap, e: Expression, portAndField: Expression): Seq[String] =
    e.tpe match {
      case (_: GroundType) =>
        ...
        Seq(loRef.serialize)
      case (t: BundleType) =>
        t.fields.foldLeft(Seq[String]()) { (names, f) =>
      ...
      case (t: VectorType) =>
        (0 until t.size).foldLeft(Seq[String]()) { (names, i) =>
  ...
  def lowerTypesMemExp(memDataTypeMap: MemDataTypeMap, info: Info, mname: String)(e: Expression): Seq[Expression] = {
    ...
    field.name match {
      ...
      case "addr" | "en" | "clk" | "wmode" =>
        ...
        memDataTypeMap(mem.name) match {
          ...
          case memType =>
            create_exps(mem.name, memType).map { e =>
  ...
  def lowerTypesExp(memDataTypeMap: MemDataTypeMap, info: Info, mname: String)(e: Expression): Expression =
    e match {
      case e: WRef => e
        ...
        kind(e) match {
          ...
          case MemKind =>
            ...
            exps.size match {
              ...
              case _ =>
                error(
                  "Error! lowerTypesExp called on MemKind " +
  ...
  def lowerTypesStmt(
    memDataTypeMap:  MemDataTypeMap,
    minfo:           Info,
    mname:           String,
    renames:         RenameMap,
    initializedMems: Set[(String, String)]
      ...
      case sx: DefRegister =>
        sx.tpe match {
          ...
          case _ =>
            ...
            Block((es.zip(names)).zip(inits).map {
              case ((e, n), i) =>
  ...
  def execute(state: CircuitState): CircuitState = {
    ...
    val initializedMems = state.annotations.collect {
      case m: MemoryInitAnnotation if !m.isRandomInit =>
        (m.target.encapsulatingModule, m.target.ref)
    }.toSet
    val c = state.circuit
