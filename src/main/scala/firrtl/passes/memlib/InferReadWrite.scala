...
object InferReadWritePass extends Pass {
  ...
  def getProductTerms(connects: Connects)(e: Expression): Seq[Expression] =
    e match {
      // No ConstProp yet...
  ...
  def checkComplement(a: Expression, b: Expression) =
    (a, b) match {
      // b ?= Not(a)
  ...
  def inferReadWriteStmt(connects: Connects, repl: Netlist, stmts: Statements)(s: Statement): Statement =
    s match {
...
class InferReadWrite extends Transform with DependencyAPIMigration with SeqTransformBased with HasShellOptions {
  ...
  def transforms = Seq(InferReadWritePass, CheckInitialization, InferTypes, ResolveKinds, ResolveFlows)
