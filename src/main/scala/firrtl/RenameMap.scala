...
sealed class RenameMap (
  ...
  private def _copy(chained: Option[RenameMap] = _chained): RenameMap = {
    val ret = new MutableRenameMap(chained = _chained.map(_._copy()))
...
package object renamemap {
  ...
  final class MutableRenameMap private[firrtl] (
    underlying: mutable.HashMap[CompleteTarget, Seq[CompleteTarget]] =
      mutable.HashMap[CompleteTarget, Seq[CompleteTarget]](),
    chained: Option[RenameMap] = None,
    doDistinct: Boolean = true) extends RenameMap(underlying, chained, doDistinct) {
