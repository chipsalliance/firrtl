...
class VerilogEmitter extends SeqTransform with Emitter {
  ...
  override def prerequisites =
    firrtl.stage.Forms.AssertsRemoved ++
  ...
  //;------------- PASS -----------------
  def v_print(e: Expression)(implicit w: Writer) =
    e match {
  ...
  def op_stream(doprim: DoPrim): Seq[Any] = {
    ...
    def checkArgumentLegality(e: Expression): Unit =
      e match {
    ...
    def checkCatArgumentLegality(e: Expression): Unit =
      e match {
  ...
  /**
    ...
    * @return             the render reference
    */
  def getRenderer(descriptions: Seq[DescriptionAnnotation], m: Module, moduleMap: Map[String, DefModule])(implicit
    writer:                     Writer
  ): VerilogRender = {
  ...
  class VerilogRender(
    ...
    def declare(b: String, n: String, t: Type, info: Info, preset: Expression) =
      t match {
        case tx: VectorType =>
    ...
    private def declare(b: String, n: String, t: Type, info: Info, ifdefOpt: Option[String]): Unit =
      t match {
        case tx: VectorType =>
    ...
    def regUpdate(r: Expression, clk: Expression, reset: Expression, init: Expression) = {
      def addUpdate(info: Info, expr: Expression, tabs: String): Seq[Seq[Any]] =
        expr match {
          case m: Mux =>
