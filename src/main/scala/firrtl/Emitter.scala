      ...
      def build_streams(s: Statement): Statement = s map build_streams match {
        ...
        case sx: DefMemory =>
          ...
          for (r <- sx.readers) {
            ...
            //; Read port
            assign(addr, netlist(addr), sx.info) //;Connects value to m.r.addr
          ...
          for (w <- sx.writers) {
            ...
            //; Write port
            assign(data, netlist(data), sx.info)
            assign(addr, netlist(addr), sx.info)
            assign(mask, netlist(mask), sx.info)
            assign(en, netlist(en), sx.info)
