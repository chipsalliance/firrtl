...
object RtlilEmitter extends SeqTransform with Emitter with DependencyAPIMigration {
  ...
  class RtlilRender(description:      Seq[Description],
    ...
    val netlist : mutable.LinkedHashMap[WrappedExpression, InfoExpr] = mutable.LinkedHashMap()
    val namespace : Namespace = Namespace(m)

    val portdefs : ArrayBuffer[Seq[Any]] = ArrayBuffer[Seq[Any]]()
    val declares : ArrayBuffer[Seq[Any]] = ArrayBuffer()
    val instdeclares : mutable.Map[String,InstInfo] = mutable.Map()
    val assigns : ArrayBuffer[Seq[Any]] = ArrayBuffer()
    val attachSynAssigns : ArrayBuffer[Seq[Any]] = ArrayBuffer()
    val processes : ArrayBuffer[Seq[Any]] = ArrayBuffer()
    // Used to determine type of initvar for initializing memories
    var maxMemSize: BigInt = BigInt(0)
    val initials : ArrayBuffer[Seq[Any]] = ArrayBuffer()
    val formals : ArrayBuffer[Seq[Any]] = ArrayBuffer()
    val moduleTarget : ModuleTarget = CircuitTarget(circuitName).module(m.name)
    ...
    def build_attribute(attr: String): Seq[Seq[String]] = {
    ...
    def infos_to_attr(info : Info) : Option[String] = {
    ...
    def string_to_rtlil_name(name: String) : String = {
      if(name.head == '_') {
        "$"+name
      } else {
        "\\" + name
    ...
    def ref_to_name(ref: Reference) : String = {
    ...
    def regUpdate(r: Expression, clk: Expression, reset: Expression, init: Expression) = {
      ...
      def addUpdate(info: Info, expr: Expression, tabs: Seq[String]): Seq[Seq[Any]] = expr match {
        case m: Mux =>
          ...
          lazy val _if: Seq[Seq[Any]] = Seq(Seq(tabs,eninfo),
            Seq(tabs, "switch ", SrcInfo(m.cond,eninfo).str_rep)) ++ (
          ...
          lazy val _ifNot: Seq[Seq[Any]] = Seq(Seq(tabs,eninfo),
            Seq(tabs, "switch ", SrcInfo(m.cond,eninfo).str_rep)) ++ (
    ...
    def bigint_to_str_rep(bigInt : BigInt, width: BigInt) : String = {
      if(width > 31) {
        var bigboi = bigInt
        var widthcnt = width
    ...
    case class InstInfo(val inst_name : String, val mod_name : String , val info: Info) {
    ...
    case class SrcInfo(str_rep : String, signed : Boolean, width: BigInt)
    object SrcInfo {
    ...
    def primop_to_cell(p : PrimOp): String = p match {
      case Not => "$not"
      case Neg => "$neg"
      ...
      case Div => "$div"
      case Rem => "$rem"
      case _ => throw new IllegalArgumentException("oopsie woopsie! you did a fucky wucky! primop %s shouldn't have propagated this far!".format(p.serialize));
    }

    def unary_cells = List("$not","$neg","$reduce_and","$reduce_or","$reduce_xor")
    def get_type_width(e : Type) : BigInt = { // just trust me bro, its lofirrtl
      e.asInstanceOf[GroundType].width.asInstanceOf[IntWidth].width
    }

    def emit_cell(i : Info, name : String, params: Seq[(String,String)], connections: Seq[(String,String)]): Seq[Seq[Any]] = {
    ...
    def emit_unary_cell(cell : String, src : SrcInfo, target : String, tgt_width: BigInt) : Seq[Seq[Any]] = {
      emit_cell(NoInfo,cell,
        Seq(
          ("A_SIGNED",if (src.signed) {"1"} else {"0"}),
    ...
    def emit_binary_cell(cell : String, src_a : SrcInfo, src_b : SrcInfo, target : String,tgt_width: BigInt) : Seq[Seq[Any]] = {
      emit_cell(NoInfo,cell,
        Seq(
          ("A_SIGNED",if (src_a.signed) {"1"} else {"0"}),
          ("A_WIDTH",src_a.width.toString),
          ("B_SIGNED",if (src_b.signed) {"1"} else {"0"}),
    ...
    def output_expr(n : String, d : Expression, i : Info) : Seq[Seq[Any]] = d match {
      ...
      case DoPrim(op, args, consts, _) =>
        ...
        op match {
          ...
          case Shr | Shl =>
            ...
            emit_binary_cell(prim,src0,SrcInfo(bigint_to_str_rep(consts.head,consts.head.bitLength),signed = false,consts.head.bitLength),n,get_type_width(d.tpe))
          case Dshr | Dshl =>
            val prim = if(op == Dshr) (if(src0.signed) "$sshr" else "$shr") else "$shl"
            emit_binary_cell(prim,src0,sources(1),n,get_type_width(d.tpe))
          case Cat =>
            Seq(Seq("connect ",n," { ", src0," ",sources(1)," }"))
          case Head =>
          ...
          case Pad =>
            if(src0.width >= consts.head)
              ...
              if(src0.signed)
                Seq(Seq("connect ",n,Seq(" { ","%s [%d] ".format(src0.str_rep,src0.width-1)*(consts.head - src0.width).toInt,src0," }").mkString))
              else
    ...
    def build_streams(s: Statement): Unit = {
      ...
      withoutDescription match {
        ...
        case sx: DefWire =>
          ...
          declares += Seq("wire ",sx.tpe," \\",sx.name)
        case sx: DefRegister =>
        ...
        case x @ DefMemory(i,name,tpe,depth,wlat,rlat,rd,wr,rdwr,runderw) =>
            ...
            case MemoryArrayInit(values) =>
              values.zipWithIndex.foreach {
                case (value, addr) =>
                  ...
                  initials ++= emit_cell(i,"$meminit_v2",
                    Seq(("MEMID","\"\\\\"+name+"\""),
                      ("ABITS", "32"),
                      ("WIDTH", dataWidth.toString),
                      ("WORDS", "1"),
                      ("PRIORITY", addr.toString)),
                    Seq(("ADDR",addr.toString),
          ...
          for (r <- rd) {
            ...
            val hasClk = if(rlat == 1) {"1'1"} else {"1'0"}
            ...
            assigns ++= emit_cell(i,"$memrd",
              Seq(("ABITS",get_type_width(addr.tpe).toString),
                ("MEMID","\"\\\\"+name+"\""),
                ("WIDTH",get_type_width(data.tpe).toString),
                ("CLK_ENABLE", hasClk),
                ("CLK_POLARITY","1'1"),
                ("TRANSPARENT", transparent)),
              Seq(("CLK",SrcInfo(clkSrc,i).str_rep),
                ("EN",SrcInfo(netlist(en),i).str_rep),
          ...
          for (w <- wr) {
            ...
            val memwr_enmask = namespace.newName("$memwr_enmask$"+m.name)
            declares += Seq("wire signed width ",bitWidth(data.tpe).toInt," ",memwr_enmask)
            assigns ++= emit_cell(i,"$and",Seq(
              ("A_SIGNED", "1"),
              ("B_SIGNED", "1"),
              ...
              Seq(("A",enSrc.str_rep),
                ("B",maskSrc.str_rep),
                ("Y",memwr_enmask)))
            val hasClk = if(wlat == 1) {"1'1"} else {"1'0"}
            val clkSrc = netlist(memPortField(x, w, "clk")).expr
            assigns ++= emit_cell(i,"$memwr",
              Seq(("ABITS",get_type_width(addr.tpe).toString),
                ("MEMID","\"\\\\"+name+"\""),
                ("WIDTH",get_type_width(data.tpe).toString),
                ("CLK_ENABLE", hasClk),
                ("CLK_POLARITY","1'1"),
                ("PRIORITY", "32'1")),
              Seq(("CLK",SrcInfo(clkSrc).str_rep),
                ("EN",memwr_enmask),
