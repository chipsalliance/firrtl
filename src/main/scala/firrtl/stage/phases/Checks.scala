...
object Checks extends Phase {
  ...
  def transform(annos: AnnotationSeq): AnnotationSeq = {
    ...
    /* Show a warning if the user didn't specify any emitter. This indicates they may be confused during Driver to
     * FirrtlStage migration.
     */
    if (eam.size + ec.size == 0) {
      StageUtils.dramaticWarning(
        s"""|No emitter specified. FirrtlStage (unlike with Driver) requires an explicit emitter. While allowable, did
            |you forget to include an emitter using one of the following:
            |  - a directive to emit a circuit: --emit-circuit, -E, EmitCircuitAnnotation
            |  - a directive to emit one-file-per-module: --emit-modules, -e, EmitAllModulesAnnotation""".stripMargin)
    }
