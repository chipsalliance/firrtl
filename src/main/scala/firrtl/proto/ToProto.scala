...
object ToProto {
  ...
  def writeToStreamFast(ostream: OutputStream, info: ir.Info, modules: Seq[() => ir.DefModule], main: String): Unit = {
  ...
  def convert(ruw: ir.ReadUnderWrite.Value): ReadUnderWrite =
    ruw match {
  ...
  def convert(formal: ir.Formal.Value): Formal =
    formal match {
  ...
  /** Converts a Width to a ProtoBuf Width Builder
    ...
    */
  def convert(width: ir.Width): Option[Firrtl.Width.Builder] =
    width match {
  ...
  def convert(direction: ir.Direction): Firrtl.Port.Direction =
    direction match {
