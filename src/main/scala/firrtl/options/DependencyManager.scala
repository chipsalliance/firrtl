...
trait DependencyManager[A, B <: TransformLike[A] with DependencyAPI[B]] extends TransformLike[A] with DependencyAPI[B] {
  ...
  protected def copy(
    ...
    knownObjects: ISet[B] = classToObject.values.toSet
  ): B
  ...
  private lazy val otherDependents: DiGraph[B] = {
    val edges = {
      val x = new LinkedHashMap ++ _targets
        .map(classToObject)
        .map { a =>
          a -> prerequisiteGraph.getVertices.filter(a._dependents(_))
        }
      x.values
        .reduce(_ ++ _)
        .foldLeft(x) {
  ...
  private def cyclePossible[A](a: String, diGraph: DiGraph[_])(thunk: => A): A =
    try {
      thunk
    } catch {
  ...
  private def safeConstruct[A](a: Class[_ <: A]): A =
    try {
      a.newInstance
    } catch {
  ...
  lazy val transformOrder: Seq[B] = {
    ...
    val (state, lowerers) = {
      ...
      val postprocessing: Option[B] = {
        if (missing.nonEmpty) {
          Some(this.copy(_targets.toSeq, s.toSeq))
        } else {
          None
        }
      }
  ...
  def dependenciesToGraphviz: String = {
    ...
    val nodes =
      (prerequisiteGraph + dependentsGraph + invalidateGraph + otherDependents).getVertices
...
class PhaseManager(
  ...
  val knownObjects: Set[Phase] = Set.empty
    ...
    with DependencyManager[AnnotationSeq, Phase] {
