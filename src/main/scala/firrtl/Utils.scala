...
object Utils extends LazyLogging {
  ...
  def expandRef(e: Expression): Seq[Expression] = e match {
    ...
    case ex =>
      ex.tpe match {
        case (_: GroundType) => Seq(ex)
        case (t: BundleType) =>
          t.fields.foldLeft(Seq[Expression](ex))(
            (exps, f) => exps ++ create_exps(WSubField(ex, f.name, f.tpe, times(flow(ex), f.flip)))
          )
        case (t: VectorType) =>
          (0 until t.size)
            .foldLeft(Seq[Expression](ex))((exps, i) => exps ++ create_exps(WSubIndex(ex, i, t.tpe, flow(ex))))
      }
  }
  ...
  /** Returns an inlined expression (replacing node references with values),
    ...
    */
  def inline(nodeMap: NodeMap, stop: String => Boolean = { x: String =>
    false
