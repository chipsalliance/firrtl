...
object JsonProtocol {
  class TransformClassSerializer
      extends CustomSerializer[Class[_ <: Transform]](
        format =>
          (
  ...
  class NamedSerializer
      extends CustomSerializer[Named](
        format =>
          (
            { case JString(s) => AnnotationUtils.toNamed(s) }, { case named: Named => JString(named.serialize) }
          )
      )
  class CircuitNameSerializer
      extends CustomSerializer[CircuitName](
        format =>
          (
  ...
  class ComponentNameSerializer
      extends CustomSerializer[ComponentName](
        format =>
          (
  ...
  class TransformSerializer
      extends CustomSerializer[Transform](
        format =>
          (
            {
              case JString(s) =>
  ...
  class LoadMemoryFileTypeSerializer
      extends CustomSerializer[MemoryLoadFileType](
        format =>
          (
  ...
  class TargetSerializer
      extends CustomSerializer[Target](
        format =>
          (
            { case JString(s) => Target.deserialize(s) }, { case named: Target => JString(named.serialize) }
          )
      )
  class GenericTargetSerializer
      extends CustomSerializer[GenericTarget](
        format =>
          (
  ...
  class CircuitTargetSerializer
      extends CustomSerializer[CircuitTarget](
        format =>
          (
  ...
  class ModuleTargetSerializer
      extends CustomSerializer[ModuleTarget](
        format =>
          (
  ...
  class InstanceTargetSerializer
      extends CustomSerializer[InstanceTarget](
        format =>
          (
  ...
  class ReferenceTargetSerializer
      extends CustomSerializer[ReferenceTarget](
        format =>
          (
  ...
  def deserializeTry(in: JsonInput): Try[Seq[Annotation]] =
    Try({
      val parsed = parse(in)
      ...
      def findTypeHints(classInst: Seq[JValue], requireClassField: Boolean = false): Seq[String] =
        classInst
          .flatMap({
