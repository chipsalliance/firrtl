...
object JsonProtocol {
  ...
  class TypeSerializer
      extends CustomSerializer[Type](format =>
        ({ case JString(s) => Parser.parseType(s) }, { case tpe: Type => JString(tpe.serialize) })
      )
  class ExpressionSerializer
      extends CustomSerializer[Expression](format =>
        ({ case JString(s) => Parser.parseExpression(s) }, { case expr: Expression => JString(expr.serialize) })
      )
  ...
  class PortSerializer
      extends CustomSerializer[Port](format =>
        ({ case JString(s) => Parser.parsePort(s) }, { case port: Port => JString(port.serialize) })
      )
  class DefModuleSerializer
      extends CustomSerializer[DefModule](format =>
        ({ case JString(s) => Parser.parseDefModule(s) }, { case mod: DefModule => JString(mod.serialize) })
      )
  class CircuitSerializer
      extends CustomSerializer[Circuit](format =>
        ({ case JString(s) => Parser.parse(s) }, { case cir: Circuit => JString(cir.serialize) })
      )
  class InfoSerializer
      extends CustomSerializer[Info](format =>
        ({ case JString(s) => Parser.parseInfo(s) }, { case info: Info => JString(info.serialize) })
      )
