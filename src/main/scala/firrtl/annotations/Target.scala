...
trait IsComponent extends IsMember {
  ...
  override def justPath: Seq[TargetToken] =
    path.foldLeft(Vector.empty[TargetToken]) {
...
case class ReferenceTarget(
  ...
  override def targetParent: CompleteTarget =
    component match {
      case Nil =>
        if (path.isEmpty) moduleTarget
        else {
  ...
  def leafSubTargets(tpe: firrtl.ir.Type): Seq[ReferenceTarget] =
    tpe match {
  ...
  def allSubTargets(tpe: firrtl.ir.Type): Seq[ReferenceTarget] =
    tpe match {
      case _: firrtl.ir.GroundType => Vector(this)
