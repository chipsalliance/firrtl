...
class CleanupNamedTargets extends Transform with DependencyAPIMigration {
  ...
  )(implicit references: ISet[ReferenceTarget], renameMap: RenameMap, module: ModuleTarget): Unit =
    statement match {
  ...
  private def onModule(
    module:              ir.DefModule
  )(implicit references: ISet[ReferenceTarget], renameMap: RenameMap, circuit: CircuitTarget): Unit = {
