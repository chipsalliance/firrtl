...
case class AnnotationFileNotFoundException(file: File) extends FirrtlUserException(s"Annotation file $file not found!")
case class AnnotationClassNotFoundException(className: String)
    extends FirrtlUserException(s"Annotation class $className not found! Please check spelling and classpath")
...
object AnnotationUtils {
  ...
  def toSubComponents(s: String): Seq[TargetToken] = {
    ...
    def exp2subcomp(e: ir.Expression): Seq[TargetToken] =
      e match {
  ...
  def toExp(s: String): Expression = {
    def parse(tokens: Seq[String]): Expression = {
      ...
      def buildup(e: Expression, tokens: Seq[String]): Expression =
        tokens match {
          case "[" :: tail =>
