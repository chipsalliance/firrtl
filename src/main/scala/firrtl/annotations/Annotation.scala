...
// warnings
final case class LegacyAnnotation private[firrtl] (target: Named, transform: Class[_ <: Transform], value: String)
...
private[firrtl] object LegacyAnnotation {
  ...
  def convertLegacyAnno(anno: LegacyAnnotation): Annotation = anno match {
    ...
    case LegacyAnnotation(c: ComponentName, transform, "nodedupmem!") if transform == classOf[ResolveMemoryReference] =>
      NoDedupMemAnnotation(c)
    case LegacyAnnotation(m: ModuleName, transform, "nodedup!") if transform == classOf[DedupModules] =>
      NoDedupAnnotation(m)
