...
private case class DescribedMod(description: Description, portDescriptions: Map[String, Description], mod: DefModule)
    extends DefModule
    with HasDescription {
  val info = mod.info
  val name = mod.name
...
class AddDescriptionNodes extends Transform {
  ...
  def onModule(modMap: Map[String, Seq[String]], compMaps: Map[String, Map[String, Seq[String]]])(
    ...
    val (newMod, portDesc: Map[String, Description]) = compMaps.get(mod.name) match {
      ...
      case Some(compMap) =>
        (mod.mapStmt(onStmt(compMap)), mod.ports.collect {
          case p @ Port(_, name, _, _) if compMap.contains(name) =>
            name -> DocString(StringLit.unescape(compMap(name).mkString("\n\n")))
        }.toMap)
    }

    val modDesc = modMap.get(newMod.name).map { desc =>
      DocString(StringLit.unescape(desc.mkString("\n\n")))
    }
