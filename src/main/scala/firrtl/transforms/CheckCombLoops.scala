...
object LogicNode {
  ...
  /**
    ...
    */
  def apply(e: Expression): LogicNode =
    e match {
      case r: WRef =>
        LogicNode(r.name)
      case s: WSubField =>
...
class CheckCombLoops extends Transform with RegisteredTransform with DependencyAPIMigration {
  ...
  private def getExprDeps(deps: MutableConnMap, v: LogicNode, info: Info)(e: Expression): Unit =
    e match {
  ...
  private def getStmtDeps(simplifiedModules: mutable.Map[String, AbstractConnMap], deps: MutableConnMap)(
    s:                                       Statement
  ): Unit =
    s match {
