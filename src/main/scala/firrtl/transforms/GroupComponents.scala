...
class GroupComponents extends Transform with DependencyAPIMigration {
  ...
  def applyGrouping(
    ...
    def inGroupFixExps(group: String, added: mutable.ArrayBuffer[Statement])(e: Expression): Expression =
      e match {
        case _: Literal => e
    ...
    def inTopFixExps(e: Expression): Expression =
      e match {
  ...
  def getComponentConnectivity(m: Module): MutableDiGraph[String] = {
    ...
    def onExpr(sink: WRef)(e: Expression): Expression =
      e match {
