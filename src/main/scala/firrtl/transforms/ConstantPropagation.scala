...
class ConstantPropagation extends Transform with DependencyAPIMigration with ResolvedAnnotationPaths {
  ...
  private def constPropNodeRef(r: Expression, e: Expression) = e match {
    case _: UIntLiteral | _: SIntLiteral | _: WRef | _: WSubIndex | _: WSubField => e
    case _ => r
  }
  ...
  def execute(state: CircuitState): CircuitState = {
    ...
    val dontTouches: Seq[(OfModule, Tokens)] = dontTouchRTs.map {
      case Target(_, Some(m), tokens) => m.OfModule -> tokens.map {
        case Ref(ref) => ref
