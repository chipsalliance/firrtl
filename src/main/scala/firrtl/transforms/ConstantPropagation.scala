...
object ConstantPropagation {
  private def litOfType(value: BigInt, t: Type): Literal =
    t match {
  ...
  def pad(e: Expression, t: Type) =
    (bitWidth(e.tpe), bitWidth(t)) match {
  ...
  def foldShiftRight(e: DoPrim) =
    e.consts.head.toInt match {
      case 0 => e.args.head
      case x =>
        e.args.head match {
  ...
  private abstract class ConstPropBinding[+T] {
    def resolve[V >: T](that: ConstPropBinding[V]): ConstPropBinding[V] =
      (this, that) match {
        case (x, y) if (x == y)   => x
...
class ConstantPropagation extends Transform with DependencyAPIMigration with ResolvedAnnotationPaths {
  ...
  sealed trait FoldCommutativeOp extends SimplifyBinaryOp {
    ...
    override def apply(e: DoPrim): Expression =
      (e.args.head, e.args(1)) match {
  ...
  sealed trait SimplifyReductionOp {
    ...
    /** Reduce a reduction primitive op to a simpler expression if possible
      ...
      */
    def apply(prim: DoPrim): Expression =
      prim.args.head match {
  ...
  object FoldADD extends FoldCommutativeOp {
    def fold(c1: Literal, c2: Literal) =
      ((c1, c2): @unchecked) match {
        case (_: UIntLiteral, _: UIntLiteral) =>
          UIntLiteral(c1.value + c2.value, (c1.width.max(c2.width)) + IntWidth(1))
        case (_: SIntLiteral, _: SIntLiteral) =>
          SIntLiteral(c1.value + c2.value, (c1.width.max(c2.width)) + IntWidth(1))
      }
    def simplify(e: Expression, lhs: Literal, rhs: Expression) =
      lhs match {
  ...
  private def foldConcat(e: DoPrim) =
    (e.args.head, e.args(1)) match {
  ...
  private def foldShiftLeft(e: DoPrim) =
    e.consts.head.toInt match {
      case 0 => e.args.head
      case x =>
        e.args.head match {
  ...
  private def foldDynamicShiftLeft(e: DoPrim) =
    e.args.last match {
  ...
  private def foldDynamicShiftRight(e: DoPrim) =
    e.args.last match {
      case UIntLiteral(v, IntWidth(w)) =>
  ...
  private def foldComparison(e: DoPrim) = {
        ...
        e match {
          ...
          case _ =>
            e.tpe match {
              ...
              case UIntType(IntWidth(width)) => Range(min = BigInt(0), max = BigInt(2).pow(width.toInt) - BigInt(1))
            }
        }
    ...
    def foldIfMatchingArgs(x: Expression) =
      x match {
  ...
  private def constPropPrim(e: DoPrim): Expression =
    e.op match {
  ...
  private def constPropMux(m: Mux): Expression =
    (m.tval, m.fval) match {
  ...
  private def constPropModule(
    ...
    def backPropStmt(stmt: Statement): Statement =
      stmt match {
    ...
    def constPropStmt(s: Statement): Statement = {
      ...
      stmtx match {
        ...
        case Connect(_, lref @ WRef(lname, ltpe, RegKind, _), rhs) if !dontTouches(lname) =>
          ...
          def updateNodeMapIfConstant(e: Expression): Unit =
            regConstant(e, selfBound) match {
