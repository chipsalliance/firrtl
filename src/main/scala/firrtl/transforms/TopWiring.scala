...
import firrtl._
import firrtl.ir._
import firrtl.passes.{ExpandConnects, InferTypes, Pass, ResolveFlows, ResolveKinds}
...
case class TopWiringOutputFilesAnnotation(
  ...
  outputFunction: (
    String,
    Seq[((ComponentName, Type, Boolean, Seq[String], String), Int)],
    CircuitState
  ) => CircuitState
...
class TopWiringTransform extends Transform {
  ...
  private def getSourcesMap(state: CircuitState): Map[String, Seq[(ComponentName, Type, Boolean, InstPath, String)]] = {
    ...
    val cMap = instGraph.getChildrenInstances.map {
      case (m, wdis) =>
                ...
                path.size match {
                  ...
                  case _ => {
                    val instportname = portnamesmap.get(prefix + path.tail.mkString("_")) match {
                      case Some(ipn) => ipn
                      case None => {
                        val instmod = instgraph
                          .getChildrenInstances(module.name)
                          .collectFirst {
