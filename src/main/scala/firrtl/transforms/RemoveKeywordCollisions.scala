...
import firrtl.annotations.Target.{BasicTargetType, CircuitTargetType, ComponentTargetType, ModuleTargetType, useTargetType}
import firrtl.ir
...
class RemoveKeywordCollisions(keywords: Set[String]) extends Transform {
  ...
  /** Modify a name to not conflict with a Verilog keywords while respecting existing renames and a namespace
    ...
    */
  private def onName(n: String)(implicit renames: RenameMap, ns: Namespace, scope: Option[BasicTargetType]): String = {

    // Convert a [[String]] into [[BasicTargetType]] based on the provided scope.
    def wrap(name: String, scope: Option[BasicTargetType]): BasicTargetType = {
//      if (useTargetType) {
        scope match {
...
//    }
    ...
    val namedx: Seq[BasicTargetType] = (named match {
...
//      case Seq(CircuitName(n))      => n
  ...
  private def onType(t: ir.Type)
                    (implicit renames: RenameMap,
                     ns: Namespace,
                     scope: Option[ModuleTargetType]): ir.Type = t match {
  ...
  private def onExpression(e: ir.Expression)
                          (implicit renames: RenameMap,
                           ns: Namespace,
                           scope: Option[ModuleTargetType],
                           iToM: mutable.Map[ComponentTargetType, ModuleTargetType],
...
//      }
  ...
  /** Rename a [[Port]] to avoid keyword collisions
    ...
    */
  private def onPort(p: ir.Port)(implicit renames: RenameMap, ns: Namespace, scope: Option[ModuleTargetType]): ir.Port =
  ...
  private def onModule(renames: RenameMap,
                       circuit: CircuitTargetType,
                       modType: ModuleType)
                      (m: ir.DefModule): ir.DefModule = {
    implicit val moduleNamespace: Namespace = Namespace(m)
//    implicit val scope: Option[ModuleTargetType] = Some(ModuleName(m.name, circuit))
    implicit val scope: Option[ModuleTargetType] = Some(ModuleTarget(circuit.name, m.name))
    ...
    implicit val iToM: mutable.Map[ComponentTargetType, ModuleTargetType] = mutable.Map.empty
...
//    implicit val scope: Option[CircuitTargetType] = Some(CircuitName(c.main))
    implicit val scope: Option[CircuitTargetType] = Some(CircuitTarget(c.main))
