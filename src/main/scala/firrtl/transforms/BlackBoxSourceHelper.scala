...
class BlackBoxSourceHelper extends firrtl.Transform {
      ...
      case ((acc, tdir, flistName), anno) =>
        anno match {
          case BlackBoxTargetDirAnno(dir) =>
            ...
            if (!targetDir.exists()) {
              FileUtils.makeDirectory(targetDir.getAbsolutePath)
            }
  ...
  override def execute(state: CircuitState): CircuitState = {
    ...
    val verilogSourcesOnly = (resourceFiles ++ inlineFiles).filterNot { f =>
      isHeader(f.getName())
    }
...
object BlackBoxSourceHelper {
  /** Safely access a file converting [[FileNotFoundException]]s and [[NullPointerException]]s into
    ...
    */
  private def safeFile[A](fileName: String)(code: => A) =
    try {
      code
    } catch {
