...
object WSubAccess {
  ...
  def unapply(wsa: WSubAccess): Option[(Expression, Expression, Type, Flow)] =
    Some((wsa.expr, wsa.index, wsa.tpe, wsa.flow))
}
...
private[firrtl] object InfoExpr {
  ...
  def unwrap(expr: Expression): (Info, Expression) =
    expr match {
  ...
  def map(expr: Expression)(f: Expression => Expression): Expression =
    expr match {
