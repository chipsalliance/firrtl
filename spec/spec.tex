...
To facilitate model checking and formal methods, there are three verification statements available: assert, assume and cover. Backends are free to generate the corresponding model checking constructs in the target language, but this is not required by the FIRRTL specification. For example, the Verilog emitter produces Verilog assert, assume and cover statements. However, backends should not generate any new synthesizable hardware when handling to verification statements. Backends that do not obey this rule should issue a warning.

Each type of verification statement requires a clock signal, a predicate signal, an enable signal and an explanatory message string literal. The predicate and enable signals must have single bit unsigned integer type. When an assert is violated the explanatory message may be issued as guidance. The explanatory message may be phrased as if prefixed by the words ``Verifies that...''.

\subsubsection{Assert}

The assert statement verifies that the predicate is true at all time steps when the enable is true. In other words, it verifies that enable implies predicate. 
...
The assume statement directs the model checker to disregard any states where the enable is true and the predicate is not true. In other words, it reduces the states to be checked to only those where enable implies predicate is true by definition.
...
The cover statement verifies that the predicate is true at some time step when the enable is true. In other words, it directs the model checker to find some way to make enable implies predicate true at some time step.
